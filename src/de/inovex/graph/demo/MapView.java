...
//	private static class ProductionComparator implements Comparator<Location> {
//		@Override
//		public int compare(Location object1, Location object2) {
//			return object1.production - object2.production;
//		}
//
//	}
	...
	public static class Location extends Point {
		...
		public Location(int production, int power, int x, int y) {
			super(x, y);
	...
	private LocationComparator mPointComparator = new LocationComparator();
	//private ProductionComparator mProductionComparator = new ProductionComparator();
	private PowerComparator mPowerComparator = new PowerComparator();
		...
		mCirclePaint = new Paint() {
			{
				...
				//setMaskFilter(new BlurMaskFilter(5, Blur.NORMAL)); //not supported for hardware accelerated views
			}
		};
...
//		final Typeface tf = Typeface.create(Typeface.SANS_SERIF, Typeface.BOLD);
...
//		};
	...
	public void addLocation(int production,int maxProduction, int x, int y) {
		addLocation(new Location(production,maxProduction, x, y));
	}
	 ...
	 * @param offsets will contain the new offsets
	 * @param scale scaling factor which translates location and border points into screen space
	 * @param totalMaxX maximum X value of all points (border and location)
	 * @param totalMinX minimum X value of all points
	 * @param totalMaxY maximum Y value of all points
	 * @param totalMinY minimum Y value of all points
	 */
	...
	protected void onDraw(Canvas canvas) {
		if (mLocationAdded) {
			Collections.sort(mLocations, mPointComparator);
		...
		if (mLocations.size() > 0) {
			final int maxPower = Collections.max(mLocations, mPowerComparator).power;
			final int minPower = Collections.min(mLocations, mPowerComparator).power;
			...
			for (Location p : mLocations) {
				...
				mappedPoints[1] = p.y;
				mViewPortMatrix.mapPoints(mappedPoints);
				//float r = (float) (15*Math.log(p.power) / logRadiusScale);
				double r = p.power*linearScale;
				//int c = (int) Math.round(200*Math.log(p.production) / logColorScale) +55;
				...
				if (productionRatio == 0){
					...
					mCirclePaint.setARGB(255, grey, grey, grey);
				}
...
//				if (productionRatio>0){
//					canvas.drawText(productionRatio +  "%", (float) (mappedPoints[0]+r), (float) (mappedPoints[1]-r),mTextPaint);
//				}
