package de.inovex.mindtherobot;
...
public final class Thermometer extends View {
	...
	// scale configuration
	private int totalNicks = 70;
	private float degreesPerNick = 360.0f / totalNicks;	
	private int centerDegree = 50; // the one in the top center (12 o'clock)
	...
	public void setMaxValue(int max){
		maxValue = max;
		centerDegree = (maxValue+minValue)/2;
	}
	...
	private int nickToDegree(int nick) {
		final int rawDegree = ((nick < totalNicks / 2) ? nick : (nick - totalNicks)) * 2;	
		final int shiftedDegree = rawDegree + centerDegree;
	...
	private float getRelativeTemperaturePosition() {
		if (handPosition < centerDegree) {
			return - (centerDegree - handPosition) / (float) (centerDegree - minValue);
		} else {
			return (handPosition - centerDegree) / (float) (maxValue - centerDegree);
		}
	}
